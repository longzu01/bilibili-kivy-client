...
class QRCodeLoginDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # WebView 显示二维码
        self.qr_webview = QWebEngineView()
        self.qr_webview.setMinimumHeight(300)
        qr_layout.addWidget(self.qr_webview)
    ...
    def generate_qrcode(self):
        ...
        try:
            ...
            if data.get('code') == 0:
                ...
                # 在 WebView 中显示二维码页面
                self.qr_webview.load(QUrl(f'https://passport.bilibili.com/h5-app/passport/login/scan?qrcode_key={self.qrcode_key}'))
                self.qr_status_label.setText('请使用哔哩哔哩 APP 扫描二维码')
                
                # 开始轮询检查状态
                self.poll_timer.start(3000)  # 每3秒检查一次
            else:
        ...
        try:
            ...
            if data.get('code') == 0:
                ...
                if status == 0:  # 扫码成功
                    ...
                    if cookies:
                        self.cookie = cookies.split(';')[0]  # 取第一个 Cookie
                        self.accept()
                    else:
