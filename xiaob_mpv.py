...
# -*- coding: utf-8 -*-
"""
小B - Bilibili 真·视频播放器 (mpv 引擎)
兼容 Windows 7/10/11，自动下载解码器
"""
...
class XiaobPlayer(QMainWindow):
    ...
    def __init__(self):
        ...
        # 初始化 mpv
        try:
            self.player = mpv.MPV(vo='gpu', input_default_bindings=True, input_vo_keyboard=True)
    ...
    def get_video_url(self, bvid):
        ...
        try:
            ...
            print(f"Play API Error: {data.get('message')}", file=sys.stderr)
            return None
        except Exception as e:
    ...
    def play_video(self, bvid):
        ...
        if not video_url:
            self.show_error('无法获取视频地址，可能需要登录')
            return
        ...
        if sys.platform.startswith('win'):
            self.player.window_handle = int(self.video_container.winId())
        
        # 播放视频
        self.player.play(video_url)
        print("Video started playing", file=sys.stderr)
    ...
    def keyPressEvent(self, event):
        """ESC 键返回列表"""
        if event.key() == Qt.Key_Escape and self.video_container.isVisible():
            if self.player:
                self.player.stop()
            self.video_container.hide()
