...
class BilibiliClient(QMainWindow):
    ...
    def open_video(self, url):
        ...
        self.status_label.setText(f'正在加载视频...')
    ...
    def on_web_load_finished(self, success):
        ...
        if not success:
            # 内嵌浏览器加载失败，回退到系统浏览器
            print("WebView failed, falling back to system browser", file=sys.stderr)
            self.web_view.hide()
            ...
            if reply == QMessageBox.Yes:
                ...
                self.status_label.setText('已在浏览器中打开')
            else:
                self.status_label.setText('加载失败')
        else:
            self.status_label.setText('视频加载中...')
