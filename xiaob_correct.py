...
# -*- coding: utf-8 -*-
"""
小B - Bilibili 专属浏览器（正确拦截版）
使用 QWebEnginePage.acceptNavigationRequest 在加载前拦截
兼容 Windows 7/10/11
"""
...
class RestrictedWebEnginePage(QWebEnginePage):
    ...
    def acceptNavigationRequest(self, url, navigation_type, is_main_frame):
        ...
        # 阻止其他网站
        if hostname:
            print(f"Blocked navigation to: {url.toString()}", file=sys.stderr)
            QMessageBox.warning(
                self.view(),
                '访问限制',
        ...
        return True  # 允许其他情况（如 about:blank）
